...
<template>
  <div class="space-y-6">
    ...
    <div class="space-y-3">
      <UButton @click="handleSocialRegister('google')" block size="lg" variant="outline" color="neutral"
        class="cursor-pointer">
        <Icon name="i-heroicons-globe-alt" class="w-5 h-5" />
        {{ $t('auth.register.continueWithGoogle') }}
      </UButton>
      <UButton @click="handleSocialRegister('github')" block size="lg" variant="outline" color="neutral"
        class="cursor-pointer">
    ...
    <UForm :schema="schema" :state="state" class="space-y-4" @submit="onSubmit">
      ...
      <UFormField :label="String($t('auth.register.name'))" name="name">
        <UInput v-model="state.name" type="text" :placeholder="String($t('auth.register.namePlaceholder'))" size="lg"
      ...
      <UFormField :label="String($t('auth.register.username') || 'Username')" name="username">
        <UInput v-model="state.username" type="text"
          :placeholder="String($t('auth.register.usernamePlaceholder') || 'Enter your username')" size="lg"
      ...
      <UFormField :label="String($t('auth.register.email'))" name="email">
        <UInput v-model="state.email" type="email" :placeholder="String($t('auth.register.emailPlaceholder'))" size="lg"
      ...
      <UFormField :label="String($t('auth.register.password'))" name="password">
        <UInput v-model="state.password" type="password" :placeholder="String($t('auth.register.passwordPlaceholder'))"
          size="lg" autocomplete="new-password" class="w-full">
          <template #leading>
      ...
      <UFormField :label="String($t('auth.register.confirmPassword'))" name="confirmPassword">
        <UInput v-model="state.confirmPassword" type="password"
          :placeholder="String($t('auth.register.confirmPasswordPlaceholder'))" size="lg" autocomplete="new-password"
          class="w-full">
          <template #leading>
            <Icon name="i-heroicons-lock-closed" class="w-5 h-5 text-gray-400" />
          </template>
      ...
      <!-- Submit Button -->
      <UButton type="submit" block size="lg" color="primary" :loading="isLoading" :disabled="isLoading"
        class="cursor-pointer">
