...
<template>
  <div class="space-y-6">
    ...
    <div class="space-y-3">
      <UButton @click="handleSocialLogin('google')" block size="lg" variant="outline" color="neutral"
        class="cursor-pointer">
        <Icon name="i-heroicons-globe-alt" class="w-5 h-5" />
        {{ $t('auth.login.continueWithGoogle') }}
      </UButton>
      <UButton @click="handleSocialLogin('github')" block size="lg" variant="outline" color="neutral"
        class="cursor-pointer">
    ...
    <UForm :schema="schema" :state="state" class="space-y-4" @submit="onSubmit">
      ...
      <UFormField :label="String($t('auth.login.email'))" name="email">
        <UInput v-model="state.email" type="email" :placeholder="String($t('auth.login.emailPlaceholder'))" size="lg"
      ...
      <UFormField :label="String($t('auth.login.password'))" name="password">
        <UInput v-model="state.password" type="password" :placeholder="String($t('auth.login.passwordPlaceholder'))"
          size="lg" autocomplete="current-password" class="w-full">
          <template #leading>
      ...
      <div class="flex items-center justify-between">
        <div class="flex items-center">
          <UCheckbox v-model="state.rememberMe" name="remember-me" :label="String($t('auth.login.rememberMe'))" />
        </div>
